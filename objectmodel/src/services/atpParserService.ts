...
class AtpParserService {
  ...
  /**
   * Parser un tableau Markdown au format grid
   */
  private parseMarkdownTable(lines: string[], startIndex: number): Record<string, string> {
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      // Ligne de séparation (début ou séparation entre lignes)
      if (line.startsWith('+-') || line.startsWith('+=')) {
      ...
      if (line.startsWith('|')) {
        ...
        if (cells.length >= 2) {
          // Extraire la clé (enlever les ** du markdown)
          let key = cells[0].replace(/\*\*/g, '').trim()
          let value = cells[1].replace(/\*\*/g, '').trim()
          
          // Si c'est une nouvelle clé (pas vide et pas une continuation)
          if (key !== '') {
            // Vérifier si c'est une continuation de clé (mot coupé)
            // Pattern : clé courte avec valeur vide ou quasi-vide (continuation)
            const isKeyContinuation = 
              key.length < 20 &&                    // Clé courte
              currentKey !== '' &&                  // On a déjà une clé en cours
              (value === '' ||                      // Valeur vide
               value.length < 10 ||                 // Ou très courte (markup, etc.)
               value.match(/^\[.*\]$/))             // Ou juste du markup []{.mark}
            
            if (isKeyContinuation) {
              // Combiner avec la clé en cours
              // Règles de combinaison :
              // 1. Si currentKey est très court (< 3) -> continuation de mot
              // 2. Si currentKey est court (< 4) ET se termine par voyelle/consonne inhabituelle -> continuation
              // 3. Si currentKey est un acronyme connu (AA, AAs) -> espace
              // 4. Si currentKey >= 4 caractères -> probablement un mot complet -> espace
              ...
              if (knownAcronyms.includes(currentKey)) {
                // Acronyme connu -> espace
                needSpace = true
              } else if (incompletePrefixes.includes(currentKey)) {
                // Préfixe incomplet connu -> pas d'espace
                needSpace = false
              } else if (currentKey.length >= 4) {
                // Mot probablement complet -> espace
                needSpace = true
              } else {
                // Court -> continuation
                needSpace = false
              ...
              // Ne pas changer currentValue sauf si on a une vraie valeur
            ...
            // Continuation de la valeur précédente (cellule vide)
    ...
    // Sauvegarder la dernière cellule si nécessaire
    if (currentKey && currentValue) {
      tableData[currentKey] = currentValue.trim()
    }
   ...
   * Parser un fichier Markdown ATP
   */
  private parseMarkdownFile(markdown: string): Promise<AtpTest[]> {
    try {
      ...
      while (i < lines.length) {
        ...
        // Détecter les chapitres (## Titre)
        if (line.startsWith('## ') && !line.startsWith('### ')) {
          chapterCounter++
          const title = line.substring(3).trim()
          currentChapter = { number: chapterCounter.toString(), title }
          currentSection = null

          results.push({
            _id: `chapter_${chapterCounter}_${idCounter++}`,
            type: 'chapter',
        ...
        // Détecter les sections et test cases (### Titre)
        if (line.startsWith('### ')) {
          const title = line.substring(4).trim()
          
          // Vérifier si c'est un test case (contient ACESM-XX-TCXX)
          const testMatch = title.match(/(ACESM-[A-Z0-9]+-TC\d+):?\s*(.+?)$/)
          ...
          if (testMatch) {
            ...
            // Parser le tableau si trouvé
            if (tableStartIndex > 0) {
              const tableData = this.parseMarkdownTable(lines, tableStartIndex)
              Object.assign(testCase, tableData)
            ...
            sectionCounter++
            const sectionNum = `${currentChapter?.number || '0'}.${sectionCounter}`
            currentSection = { number: sectionNum, title }
        ...
        // Détecter les tableaux de procédures (Table X -- Titre)
        if (line.startsWith('Table ') && line.includes('--')) {
          // Extraire le titre du tableau
          const tableTitleMatch = line.match(/Table\s+(?:\d+\s+)?--\s+(.+)/)
          const tableTitle = tableTitleMatch ? tableTitleMatch[1].trim() : ''
          ...
          if (tableStartIndex > 0) {
            ...
            // Vérifier si c'est une procédure (pas un test case)
            if (tableData.tableData && tableData.tableData['**Procedure**']) {
              const procedureName = tableData.tableData['**Procedure**'] || tableTitle
              
              results.push({
                _id: `procedure_${procedureName.replace(/[^a-zA-Z0-9]/g, '_')}_${idCounter++}`,
                type: 'procedure',
                title: procedureName,
                references: tableData.tableData['References'] || '',
                testPurpose: tableData.tableData['Test purpose'] || '',
                procedureBody: tableData.tableData['Procedure body'] || '',
                chapter: currentChapter ? currentChapter.number : null,
                section: currentSection ? currentSection.number : null,
                timestamp: new Date().toISOString(),
                tableColumns: ['Field', 'Value'],
      ...
      console.error('Error parsing Markdown file:', error)
      throw new Error('Erreur lors de la lecture du fichier Markdown')
    }
  }
